...
module Jekyll
  module MarkdownAlerts
    ...
    ALERT_PATTERN = /^>\s*\[!(#{TYPES.join("|")})\]\s*$/.freeze
    ...
    def self.transform(content)
      ...
      while index < lines.length
        ...
        if (fence_match = line.match(FENCE_PATTERN))
          marker = fence_match[1]
          fence = fence.nil? ? marker[0] : nil if fence.nil? || marker.start_with?(fence)
          transformed << line
          index += 1
          next
        end

        alert_match = fence.nil? && line.match(ALERT_PATTERN)
        unless alert_match
          transformed << line
          index += 1
          next
        ...
        type = alert_match[1]
        transformed << "> #{title_markup(type)}\n"
        index += 1

        while index < lines.length && lines[index].start_with?(">")
          transformed << lines[index]
          index += 1
        end

        transformed << "\n" unless transformed.last&.end_with?("\n")
        transformed << "{: .markdown-alert .markdown-alert-#{type.downcase}}\n"
      end

      transformed.join
    end

    def self.title_markup(type)
      label = type.capitalize
      icon_data = ICONS.fetch(type)
      icon = %(<svg data-component="Octicon" class="octicon octicon-#{icon_data[:name]} markdown-alert-icon" viewBox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path d="#{icon_data[:path]}"></path></svg>)
      %(<span class="markdown-alert-title">#{icon}#{label}</span>)
    end
  end
end

Jekyll::Hooks.register %i[pages documents], :pre_render do |item|
  next unless item.respond_to?(:content) && item.output_ext == ".html"

  item.content = Jekyll::MarkdownAlerts.transform(item.content)
end
